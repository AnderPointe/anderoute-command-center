...
import {
  CARRIER_PLANS, CARRIER_SUBSCRIPTIONS, CARRIER_REVENUE_EVENTS,
  MARKETPLACE_FEE_SETTINGS, MONETIZATION_KPIS,
...
export const Route = createFileRoute("/v35/marketplace-monetization")({
  ...
  component: () => {
    const k = MONETIZATION_KPIS;
    return (
      <V35Page icon={<Coins className="size-6 text-amber-300" />} title="Carrier Marketplace Monetization"
        blurb="Subscription plans, transaction fees, premium visibility, marketplace revenue events, and monetization KPIs. Settlement processing is placeholder.">
        <div className="grid gap-3 md:grid-cols-4">
          {[
            ["MRR", `$${k.mrr.toLocaleString()}`],
            ["Carrier MRR", `$${k.carrier_mrr.toLocaleString()}`],
            ["API MRR", `$${k.api_mrr.toLocaleString()}`],
            ["Take rate", `${(k.take_rate * 100).toFixed(2)}%`],
            ["ARPU", `$${k.arpu}`],
            ["LTV (placeholder)", `$${k.ltv_placeholder.toLocaleString()}`],
            ["Gross margin", `${Math.round(k.gross_margin * 100)}%`],
            ["Fee revenue MTD", `$${k.fee_revenue_mtd.toLocaleString()}`],
          ].map(([l, v]) => (
            <Card key={String(l)} className="border-white/10 bg-white/[0.02] p-3">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">{l}</div>
              <div className="mt-1 text-lg font-semibold">{v as any}</div>
            </Card>
          ))}
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <Card className="border-white/10 bg-white/[0.02] p-4">
            ...
            <table className="mt-2 w-full text-sm">
              ...
              <tbody>{CARRIER_SUBSCRIPTIONS.map((s) => (
                <tr key={s.id} className="border-t border-white/10"><td className="p-1">{s.carrier}</td><td className="p-1">{s.plan}</td><td className="p-1"><Badge variant="outline" className={s.status === "trial" ? "border-sky-500/40 text-sky-300" : "border-emerald-500/40 text-emerald-300"}>{s.status}</Badge></td><td className="p-1">{s.renewed}</td></tr>
              ))}</tbody>
            </table>
          </Card>
          <Card className="border-white/10 bg-white/[0.02] p-4">
            <h3 className="text-sm font-semibold">Fee settings</h3>
            <ul className="mt-2 space-y-1 text-sm">{MARKETPLACE_FEE_SETTINGS.map((f) => (
        ...
        <Card className="border-white/10 bg-white/[0.02] p-4">
          ...
          <table className="mt-2 w-full text-sm">
            ...
            <tbody>{CARRIER_REVENUE_EVENTS.map((e) => (
