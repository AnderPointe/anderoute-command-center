...
import { V4_READINESS, LAUNCH_MILESTONES, LAUNCH_BLOCKERS, LAUNCH_RISKS } from "@/v4/data/mockPhase21";
    ...
    <V4Page icon={<Command className="size-6 text-sky-300" />} title="Enterprise Launch Command Center"
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        <div className="flex items-end justify-between">
          ...
          <Badge variant="outline" className="border-amber-400/40 text-amber-300">3 blockers</Badge>
        </div>
        ...
        <Card className="border-white/10 bg-white/[0.02] p-4">
          ...
          <ul className="mt-2 space-y-2 text-sm">{LAUNCH_MILESTONES.map(m => (
            <li key={m.id} className="flex items-center justify-between gap-2">
              ...
              <Badge variant="outline" className="border-white/15 text-xs">{m.status}</Badge>
            </li>))}
          </ul>
        </Card>
        <Card className="border-white/10 bg-white/[0.02] p-4">
