  ...
  component: () => (
    <V4Page icon={<ListChecks className="size-6 text-sky-300" />} title="V4 Demo Flow"
      ...
      <Card className="border-white/10 bg-white/[0.02] p-4">
        <ol className="space-y-2 text-sm">{DEMO_STEPS_V4.map((s,i) => (
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <Card className="border-white/10 bg-white/[0.02] p-4">
          ...
          <ul className="mt-2 space-y-1 text-xs">{EDGE_FN_PLAN_V4.map(f => (
            <li key={f.fn} className="rounded border border-white/10 bg-black/20 p-2">
              <span className="font-mono text-sky-300">{f.fn}</span>
              <div className="text-muted-foreground">{f.runtime} · {f.reason}</div>
            </li>))}
          </ul>
        </Card>
      </div>
