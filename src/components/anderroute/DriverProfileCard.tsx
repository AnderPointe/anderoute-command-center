...
import type { Driver, Shipment } from "@/types/logistics";
  ...
  return (
    <div className="rounded-[2rem] bg-slate-950 p-6 text-white shadow-sm">
      ...
      <div className="mt-6 flex items-center gap-5">
        ...
        <div className="space-y-1.5 text-sm text-slate-300">
          ...
          <p className="text-xs text-slate-400">
            Assignment · <span className="text-white">{shipment.id}</span>
          </p>
      ...
      <div className="mt-3 space-y-3">
        <RouteStop label="Pickup" value={shipment.pickup_address} color="bg-teal-400" />
        <RouteStop label="Dropoff" value={shipment.dropoff_address} color="bg-orange-400" />
      </div>
    </div>
  );
}
